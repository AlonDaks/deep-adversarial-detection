...
def data_resnet():
    file_names = ['ILSVRC2012_val_000{0:05d}.JPEG'.format(i+1) for i in range(5000)]
    labels = np.loadtxt('labels.txt')
    images = np.empty((5000, 3, FLAGS.img_rows, FLAGS.img_cols))
    for i in range(5000):
    ...
    X_train, Y_train = images[:4000,:,:,:], labels[:4000]
    X_test, Y_test = images[4000:,:,:,:], labels[4000:]
...
def main(argv=None):
    ...
    X_train_normal= X_train_split[:3400,:,:,:]
    X_train_adv, = batch_eval(sess, [x], [adv_x], [X_train_split[3400:,:,:,:]])
    ...
    np.save('adv_train.npy', np.array([0]*3400 + [1]*1600))
    print "Generated Training Data"

    X_test_normal= X_test_split[:850,:,:,:]
    X_test_adv, = batch_eval(sess, [x], [adv_x], [X_test_split[850:,:,:,:]])
    ...
    np.save('adv_test.npy', np.array([0]*850 + [1]*150))
